...
export function replaceHTMLElementText(element: HTMLElement, newText: string, currentText?: string) {
    if (element == null) {
    ...
    if (element.innerHTML.trim() === _currentText.trim()) {
        ...
        if (element.children.length > 0) {
            for (let i = 0; i < element.children.length; i++) {
                const result = replaceHTMLElementText(element.children[i] as HTMLElement, newText, _currentText);
                if (result == true) {
                    return true;
                }
            }
