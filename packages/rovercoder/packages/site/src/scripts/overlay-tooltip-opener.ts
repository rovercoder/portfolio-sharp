...
import { ManagedLifecycleObjectComponent } from "./global.types.js";
import { destroyManagedLifecycleObjectComponent } from "./utilities-lifecycle.js";

export function tooltipOpenerInitializer(args: { triggerElement: HTMLElement, tooltipOverlayRawSource: HTMLElement }): { [componentKey: string]: ManagedLifecycleObjectComponent } | undefined {
    
    if (args == null || typeof args !== 'object') {
        return;
    }

    const managedLifecycleObjectComponents: { [componentKey: string]: ManagedLifecycleObjectComponent } = {};
    ...
    let tooltipOverlay: HTMLElement | undefined;
    const tooltipOuterElementKey = 'tooltipOuterElement';
    let tooltipOuterElementIsFocused: boolean = false;
    let triggerElementIsFocused: boolean = false;
    let tooltipOverlayEventTypes: string[] = [];
    ...
    const openTooltipOverlay = (triggerElement: HTMLElement, triggerElementEventType: string) => {
    ...
    const closeTooltipOverlay = (): boolean => {
    ...
    const triggerElementOpenTooltipOverlayKeydownFn = (evt: KeyboardEventInit) => closeTooltipOverlay() || evt.key?.toLowerCase() !== 'enter' || openTooltipOverlay(triggerElement, 'keydown');
