import { ManagedLifecycleObject, ManagedLifecycleObjectComponent, ManagedLifecycleObjectGroupInitializeExtraArguments, OverlaysObjectGroup } from "./global.types.js";
...
import { ensureHTMLElementOrNull, ensureHTMLElementThrowOrNull, isHTMLElement, parseCssPixels } from "./utilities-general.js";
...
function getInitializeTooltipOverlayObjectsGroup(): OverlaysObjectGroup {
...
function initializeTooltipOverlay(overlayElement: HTMLElement, extra?: ManagedLifecycleObjectGroupInitializeExtraArguments) {
    ...
    const stateEntryComponents: { [componentKey: string]: ManagedLifecycleObjectComponent } = {};
    ...
    const tooltipPositionsState: (TooltipPositionCalculationFunctionResponse & { 
    ...
    const initTooltipCalculationsContainer = () => {
        ...
        const getTooltipHiddenForPositionOuterElementKey = (tooltipPosition: [string] | [string, string]) => `tooltipHiddenForPositionOuterElement-position[${getTooltipPositionString(tooltipPosition)}]`;
        ...
        for (const tooltipPosition of tooltipPositionsRequired) {
            ...
            if (result != null) {
                const tooltipPositionState: typeof tooltipPositionsState[number] = {
    ...
    interface ElementDimensions { width: number, height: number, x: number, y: number };

    const checkDimensionsChanged = (elementDimensions1: ElementDimensions, elementDimensions2: ElementDimensions): boolean =>
        (elementDimensions1.width !== elementDimensions2.width
    ...
    const getDimensionsFromDomRect = (domRect: DOMRect): ElementDimensions => ({ width: domRect.width, height: domRect.height, x: domRect.x, y: domRect.y });

    let overlayContainerInnerElementDimensions: ElementDimensions = getDimensionsFromDomRect(overlayTooltipInnerElement.getBoundingClientRect());

    let tooltipHiddenForSizeOuterElementDimensions: ElementDimensions | undefined;

    let tooltipHiddenForSizeTooltipElementDimensions: ElementDimensions | undefined;

    let tooltipSpaceAwayFromTriggerElementElementDimensions: ElementDimensions | undefined;

    const triggerElement = extra.trigger.element;
    const triggerElementEventType = extra.trigger.eventType;

    let triggerElementDimensions: ElementDimensions = getDimensionsFromDomRect(triggerElement.getBoundingClientRect());
    ...
    interface TooltipPositionCalculationFunctionArgsTooltipHiddenForSize {
        tooltipHiddenForSizeOuterElementDimensions: ElementDimensions;
        tooltipHiddenForSizeTooltipElementDimensions: ElementDimensions;
    }

    interface TooltipPositionCalculationFunctionArgsTooltipHiddenForSizeMargin {
        tooltipHiddenForSizeOuterElementDimensions: ElementDimensions;
        tooltipHiddenForSizeTooltipElementDimensions: ElementDimensions;
    }

    interface TooltipPositionCalculationFunctionArgs extends TooltipPositionCalculationFunctionArgsTooltipHiddenForSize {
        tooltipOuterElement: HTMLElement;
        tooltipSpaceAwayFromTriggerElementDimensions: ElementDimensions;
        triggerElementDimensions: ElementDimensions;
        overlayContainerInnerElementDimensions: ElementDimensions;
        tooltipMarginClosestToTriggerElementEnabled: boolean;
    }

    interface TooltipPositionCalculationFunctionScrollViewVisibleResponse {
        scrollViewVisibleScore: number;
        scrollViewVisibleHorizontallyScore: number;
        scrollViewVisibleVerticallyScore: number;
    }

    interface TooltipPositionCalculationFunctionResponse extends TooltipPositionCalculationFunctionScrollViewVisibleResponse {
        top: number;
        left: number;
        bottom: number;
        right: number;
    ...
    interface TooltipPositionEntry {
        position: [string] | [string, string];
        function: (args: TooltipPositionCalculationFunctionArgs) => TooltipPositionCalculationFunctionResponse;
    }

    const tooltipPositionCalculationFunctionArgsChecker = (args: TooltipPositionCalculationFunctionArgs): boolean => {
        return args != null
            && typeof args === 'object'
    ...
    const calculateFromOverlayLeft = (left: number, overlayContainerInnerElementDimensions: ElementDimensions): number => {
        return left - overlayContainerInnerElementDimensions.x;
    }

    const calculateFromOverlayRight = (left: number, overlayContainerInnerElementDimensions: ElementDimensions): number => {
        return (overlayContainerInnerElementDimensions.x + overlayContainerInnerElementDimensions.width) - left;
    }

    const calculateFromOverlayTop = (top: number, overlayContainerInnerElementDimensions: ElementDimensions): number => {
        return top - overlayContainerInnerElementDimensions.y;
    }

    const calculateFromOverlayBottom = (top: number, overlayContainerInnerElementDimensions: ElementDimensions): number => {
        return (overlayContainerInnerElementDimensions.y + overlayContainerInnerElementDimensions.height) - top;
    }

    const getTooltipPositionCalculationFunctionScrollViewVisibleScoreResponse = (args: TooltipPositionCalculationFunctionArgs): TooltipPositionCalculationFunctionScrollViewVisibleResponse => {
        if (args == null) {
        ...
        const _getScrollViewVisibleScoreResponseRecursive = (element: Element | null | undefined, divisor: number): TooltipPositionCalculationFunctionScrollViewVisibleResponse => {
            if (element == null) {
                return {
            ...
            let _scrollViewVisibleScoreChildren: { areaFactor: number, score: number }[] = [];
            let _scrollViewVisibleScoreHorizontallyChildren: { areaFactor: number, score: number }[] = [];
            let _scrollViewVisibleScoreVerticallyChildren: { areaFactor: number, score: number }[] = [];
            for (let i = 0; i < element.children.length; i++) {
                const childElement = ensureHTMLElementOrNull(element.children.item(i));
                const { 
                    scrollViewVisibleScore: _scrollViewVisibleScore, 
                    scrollViewVisibleHorizontallyScore: _scrollViewVisibleHorizontallyScore, 
                    scrollViewVisibleVerticallyScore: _scrollViewVisibleVerticallyScore 
                } = _getScrollViewVisibleScoreResponseRecursive(childElement, childDivisor);
    ...
    const getTooltipPositionCalculationFunctionResponse = (args: TooltipPositionCalculationFunctionArgs): TooltipPositionCalculationFunctionResponse => {
        if (args == null) {
            throw Error('Invalid arguments for getTooltipPositionCalculationFunctionResponse method!');
        }
    ...
    const getTooltipFromOuterElement = (tooltipOuterElement: HTMLElement): HTMLElement | null => {
        return ensureHTMLElementThrowOrNull(tooltipOuterElement.querySelector('.tooltip'), 'Tooltip element is invalid!');
    }

    const updateTooltipOuterElementPosition = (tooltipOuterElement: HTMLElement, overlayContainerInnerElementDimensions: ElementDimensions, positionArgs: { top?: number, left?: number, bottom?: number, right?: number }, options?: { centerX?: { x: number, exact: boolean, args: TooltipPositionCalculationFunctionArgsTooltipHiddenForSizeMargin }, centerY?: { y: number, exact: boolean, args: TooltipPositionCalculationFunctionArgsTooltipHiddenForSizeMargin } }) => {
        ...
        const returnInCssPixels = (value: number | null | undefined) => value != null ? `${value}px` : '';
        let positionChanged = false;
        
        let top: number | undefined = positionArgs.top;
        let left: number | undefined = positionArgs.left;
        let bottom: number | undefined = positionArgs.bottom;
        let right: number | undefined = positionArgs.right;
    ...
    const tooltipPositionCalculationFunctionArgsTooltipHiddenForSizeMarginValidate = (args: TooltipPositionCalculationFunctionArgsTooltipHiddenForSizeMargin | undefined | null): boolean => {
    ...
    const getTooltipLeftMargin = (args: TooltipPositionCalculationFunctionArgsTooltipHiddenForSizeMargin): number => {
    ...
    const getTooltipTopMargin = (args: TooltipPositionCalculationFunctionArgsTooltipHiddenForSizeMargin): number => {
        if (!tooltipPositionCalculationFunctionArgsTooltipHiddenForSizeMarginValidate(args)) {
    ...
    const getTooltipRightMargin = (args: TooltipPositionCalculationFunctionArgsTooltipHiddenForSizeMargin): number => {
        if (!tooltipPositionCalculationFunctionArgsTooltipHiddenForSizeMarginValidate(args)) {
    ...
    const getTooltipBottomMargin = (args: TooltipPositionCalculationFunctionArgsTooltipHiddenForSizeMargin): number => {
        if (!tooltipPositionCalculationFunctionArgsTooltipHiddenForSizeMarginValidate(args)) {
    ...
    const _tooltipPositionTopDimensionsTopAndBottom = (args: TooltipPositionCalculationFunctionArgs): { top: number, bottom: number } => {
    ...
    const _tooltipPositionLeftDimensionsLeftAndRight = (args: TooltipPositionCalculationFunctionArgs): { left: number, right: number } => {
        if (tooltipPositionCalculationFunctionArgsChecker(args) === false) {
            throw new Error('Invalid arguments for tooltip position calculation function!');
        }
    ...
    const _tooltipPositionBottomDimensionsTopAndBottom = (args: TooltipPositionCalculationFunctionArgs): { top: number, bottom: number } => {
        if (tooltipPositionCalculationFunctionArgsChecker(args) === false) {
            throw new Error('Invalid arguments for tooltip position calculation function!');
        }
    ...
    const _tooltipPositionRightDimensionsLeftAndRight = (args: TooltipPositionCalculationFunctionArgs): { left: number, right: number } => {
        if (tooltipPositionCalculationFunctionArgsChecker(args) === false) {
            throw new Error('Invalid arguments for tooltip position calculation function!');
        }
    ...
    const getTooltipPositionString = (tooltipPosition: [string] | [string, string]) => tooltipPosition.join('-');

    const tooltipPositions: TooltipPositionEntry[] = [
        {
            position: ['center', 'top'],
            function: (args) => {
                if (tooltipPositionCalculationFunctionArgsChecker(args) === false) {
    ...
    const getTooltipScrollFactorCalculationForChildrenPercentage = (): number => {
    ...
    const getTooltipMarginClosestToTriggerElementEnabled = (): boolean => {
    ...
    const getTooltipPositionsRequired = (): ([string] | [string, string])[] => {
        const tooltipPositionsAll = getTooltipPositionsAll();
    ...
    const getTooltipPositionsAll = (): ([string] | [string, string])[] => {
        return tooltipPositions.map(x => x.position);
    };

    const getTooltipPositionInternal = (tooltipPosition: string): [string] | [string, string] | undefined => {
        return tooltipPositions.find(x => matchTooltipPosition(tooltipPosition, x.position))?.position;
    };

    const matchTooltipPosition = (tooltipPosition: string | [string] | [string, string], checkPosition: [string] | [string, string]): boolean => {
        ...
        if (tooltipPosition == null || tooltipPosition.toString().trim().length === 0 || checkPosition == null || typeof checkPosition !== 'object' || !Array.isArray(checkPosition) || (checkPosition as any[]).length === 0 || !checkPosition.every(x => typeof x === 'string')) {
            return false;
        }
    ...
    interface ElementIntersectionDimensionResult { 
        full: number;
        additionalPartial: { 
            distance: number, 
    ...
    function getElementIntersectionDimensions(args: { targetElement: HTMLElement, targetContainerElement?: HTMLElement, ignoreOverlappingElements?: HTMLElement[], sampleCount?: number }): { top: ElementIntersectionDimensionResult, bottom: ElementIntersectionDimensionResult, left: ElementIntersectionDimensionResult, right: ElementIntersectionDimensionResult } | undefined {
        ...
        const targetElement: HTMLElement = args.targetElement;
        const targetContainerElement: HTMLElement | undefined = args.targetContainerElement;
        const ignoreOverlappingElements: HTMLElement[] | undefined = args.ignoreOverlappingElements;
        ...
        const plot: (Element | undefined | null)[][] = Array.from({ length: xDivisions + 1 }).map((_) => Array.from({ length: yDivisions + 1 }));
        ...
        const isOutsideContainerRect = (dimensions: { x: number, y: number }): boolean => {
        ...
        for (let i = 0; i <= xDivisions; i++) {
            for (let j = 0; j <= yDivisions; j++) {
                ...
                let elementAtPoint: HTMLElement | Element | undefined = undefined;
        ...
        const averageFn = (array: number[]) => array.reduce((a, b) => a + b) / array.length;

        const getHorizontalDimensionResultLeftNotRight = (leftNotRight: boolean) => {
        ...
        const getVerticalDimensionResultTopNotBottom = (topNotBottom: boolean) => {
            const result = { full: 0, additionalPartial: { distance: 0, saturationPercentage: 0 } };
            const additionalPartialPercentages = [];
        ...
        const results: { top: ElementIntersectionDimensionResult, bottom: ElementIntersectionDimensionResult, left: ElementIntersectionDimensionResult, right: ElementIntersectionDimensionResult } = { 
    ...
    const resetTooltipPosition = () => {
        ...
        const triggerElementIntersectionDimensionsPositions = ['left', 'right', 'top', 'bottom'] as const;
        ...
        const tooltipPositionsStatesFiltered = tooltipPositions.filter(tooltipPosition => 
            triggerElementIntersectionDimensions == null 
            || !triggerElementIntersectionDimensionsPositions.some(pos => triggerElementIntersectionDimensions[pos].full > 0 && tooltipPosition.includes(pos))
        ).map(tooltipPosition => tooltipPositionsState.find(x => matchTooltipPosition(tooltipPosition, x.position)))
        .filter(x => x != null)
        ...
        const tooltipPositionsStatesFilteredSorted = Array.from(tooltipPositionsStatesFiltered).sort((item1, item2) => { 
            ...
            if (triggerElementIntersectionDimensions != null) {
                const distanceAdjusted: { position: typeof triggerElementIntersectionDimensionsPositions[number], valueFull: number, valuePartial: number }[] = triggerElementIntersectionDimensionsPositions.map((position) => ({
                    position: position,
